...
def uni_bleu(references, sentence):
    ...
    # Calculate clipped precision (min between candidate count and max reference count)
    clipped_count = sum(min(sentence_counts[word], max_ref_counts[word]) for word in sentence_counts)
    ...
    bp = 1.0 if sentence_len >= closest_ref_len else math.exp(1 - closest_ref_len / sentence_len)
