...
class NeuralNetwork:
    ...
    def train(self, X, Y, iterations=5000, alpha=0.05, verbose=True, 
        ...
        Args:
            ...
            graph: boolean that defines whether to graph training data
            step: step size for verbose and graph outputs
        ...
        # Lists to store cost values for graphing
        costs = []
        iteration_list = []
        ...
        for i in range(1, iterations + 1):
            ...
            # Handle verbose and graph outputs
            if (verbose or graph) and (i % step == 0 or i == iterations):
                # Calculate cost AFTER gradient descent update
                _, A2_updated = self.forward_prop(X)
                current_cost = self.cost(Y, A2_updated)
                if verbose:
