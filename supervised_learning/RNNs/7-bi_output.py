...
Module implements a bidirectional recurrent neural network cell that processes
sequences in both forward and backward directions, then combines the hidden
states to produce outputs.
"""
...
class BidirectionalCell:
    """
    Bidirectional RNN cell that processes data in
    both forward and backward directions.

    Cell maintains separate hidden state dimensions for forward and backward
    passes, and combines them to generate outputs using a shared output layer.

    Attributes:
        Whf (np.ndarray): Weight matrix forward hidden state of shape (i+h, h)
        Whb (np.ndarray): Weight matrix backward hidden state of shape (i+h, h)
    ...
    def backward(self, h_next, x_t):
        ...
        Args:
            h_next (np.ndarray): Next hidden state (in backward pass)
                                 of shape (m, h)
            x_t (np.ndarray): Input data at current time step of shape (m, i)

        Returns:
            np.ndarray: Previous hidden state (backward direction)
                        of shape (m, h)
        """
    ...
    def output(self, H):
        ...
        Applies an affine transformation followed by
        softmax activation to convert the concatenated
        forward and backward hidden states into probability distributions.

        Args:
            H (np.ndarray): Concatenated hidden states of shape (t, m, 2*h)
                           where t is the number of time step, m is batch size,
                           and 2*h combines forward & backward hidden dimension
        Returns:
