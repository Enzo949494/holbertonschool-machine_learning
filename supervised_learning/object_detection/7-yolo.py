...
class Yolo:
    ...
    def predict(self, folder_path):
        ...
        # Run prediction on all images at once
        ...
        for i in range(len(images)):
            # Get outputs for this specific image
            image_outputs = [output[i] for output in outputs]

            # Process outputs for this image
            boxes, box_confidences, box_class_probs = \
                self.process_outputs(image_outputs, image_shapes[i])
            ...
            # Store prediction as tuple
            predictions.append((boxes, box_classes, box_scores))

            # Get only the filename without path for display
